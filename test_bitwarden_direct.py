...
if env_file.exists():
    ...
    if "BWS_ACCESS_TOKEN=" in env_content and "#BWS_ACCESS_TOKEN" not in env_content.split('\n')[7]:
...
try:
    ...
    print(f"   ✓ BitwardenSettingsSource class available")
    print(f"   ✓ Source priority: init → BitwardenSettingsSource → env → dotenv")
    ...
    if actual_password == local_fallback:
        ...
        print(f"   (Bitwarden may be providing same value, or using fallback)")
    else:
