...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log('[Omega_KG] Service worker received message: - background.js:9', message.type);
  
  if (message.type === 'CAPTURE_CONVERSATION') {
    saveToLocalhost(message.data)
      .then(response => {
        console.log('[Omega_KG] ✅ Captured successfully: - background.js:14', response);
        sendResponse({ success: true, response });
      })
      .catch(error => {
        console.error('[Omega_KG] ❌ Capture failed: - background.js:18', error);
...
async function saveToLocalhost(data) {
  ...
  try {
    ...
    console.error('[Omega_KG] ❌ Server error: - background.js:57', error.message);
    throw error;
  }
}
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'health-check') {
    fetch('http://localhost:8765/health')
      .then(r => r.json())
      .then(data => console.log('[Omega_KG] Server status: - background.js:69', data.status))
      .catch(() => console.warn('[Omega_KG] Server offline - background.js:70'));
  }
});