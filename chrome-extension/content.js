...
class ChatCapture {
  ...
  detectPlatform() {
    ...
    if (hostname.includes('github.com') && url.includes('copilot')) return 'GitHub_Copilot';
    if (hostname.includes('qwen.ai') || hostname.includes('tongyi.aliyun.com')) return 'Qwen';
    if (hostname.includes('z.ai')) return 'Z.ai';
    if (hostname.includes('kimi.com') || hostname.includes('kimi.moonshot')) return 'Kimi';
    if (hostname.includes('deepseek.com')) return 'DeepSeek';
    if (hostname.includes('mistral.ai')) return 'Mistral';
    
    console.log(`[Omega_KG] Unknown platform  hostname: ${hostname} - content.js:26`);
    return 'unknown';
  }
  ...
  extractMessages() {
    ...
    const selectors = {
      'Claude.ai': {
        ...
        isUser: (el) => el.classList.contains('font-user-message') || 
                       el.closest('[data-testid*="user"]') !== null,
        getText: (el) => {
          // Try multiple selectors for message content
          const content = el.querySelector('[class*="font-claude-message"]') || 
      ...
      'Gemini': {
        ...
        messages: 'message-content.model-response-text, .conversation-container, user-query',
        isUser: (el) => {
          return el.tagName.toLowerCase() === 'user-query' ||
      ...
      'Perplexity': {
        ...
        isUser: (el) => el.closest('[class*="UserMessage"]') !== null ||
    ...
    if (!config) {
      console.warn(`[Omega_KG] No selector config for platform: ${this.platform} - content.js:119`);
      return [];
    }
    ...
    if (messageElements.length === 0) {
      console.warn(`[Omega_KG] No messages found with selectors, trying fallback... - content.js:127`);
      // Try to find any text content that looks like messages
      messageElements = this.fallbackExtraction();
    }
    
    const messages = [];
    console.log(`[Omega_KG] Found ${messageElements.length} message elements on ${this.platform} - content.js:133`);
    ...
    messageElements.forEach((element, index) => {
      try {
        ...
        console.warn(`[Omega_KG] Error extracting message ${index}: - content.js:151`, err);
      }
    });

    console.log(`[Omega_KG] Extracted ${messages.length} messages from ${messageElements.length} elements - content.js:155`);
    return messages;
  }
  ...
  fallbackExtraction() {
    ...
    console.log(`[Omega_KG] Fallback found ${candidates.length} potential message containers - content.js:178`);
  ...
  async captureConversation() {
    ...
    console.log(`[Omega_KG] Attempting capture: - content.js:220`, {
    ...
    if (messages.length === 0) {
      console.warn('[Omega_KG] No messages extracted, skipping capture - content.js:227');
      return;
    }
    ...
    if (this.conversationCache.has(conversationHash)) {
      console.log('[Omega_KG] Conversation already captured (hash collision), skipping - content.js:236');
      return;
    }
    ...
    try {
      ...
      if (response?.success) {
        ...
        console.log(`[Omega_KG] ✅ Captured ${messages.length} messages from ${this.platform} - content.js:263`);
        this.showNotification('Conversation captured!', 'success');
      } else {
        console.error('[Omega_KG] ❌ Capture failed: - content.js:266', response?.error);
        this.showNotification('Capture failed - check server', 'error');
      }
    } catch (error) {
      console.error('[Omega_KG] ❌ Error sending to background: - content.js:270', error);
  ...
  debugDOM() {
    ...
    console.log('=== Omega_KG Debug Mode === - content.js:372');
    console.log('Platform: - content.js:373', this.platform);
    console.log('URL: - content.js:374', window.location.href);
    ...
    console.log(`Found ${messageElements.length} elements with messagerelated attributes - content.js:381`);
    ...
    console.log('\n=== Selector Patterns Found === - content.js:396');
    Array.from(patterns.entries())
      .sort((a, b) => b[1].count - a[1].count)
      .slice(0, 10)
      .forEach(([pattern, data]) => {
        console.log(`${data.count}x: ${pattern} - content.js:401`);
        console.log('Example text: - content.js:402', data.example.textContent.substring(0, 100));
      });
    
    console.log('\n💡 Copy these selectors and update  extractMessages() - content.js:405');
    console.log('=== End Debug === - content.js:406');
...
if (capture.platform !== 'unknown') {
  ...
  console.log(`[Omega_KG] Initialized for ${capture.platform} - content.js:418`);
} else {
  console.warn('[Omega_KG] Unknown platform, capture disabled - content.js:420');
}
...
window.addEventListener('load', () => {
  console.log('[Omega_KG] Page loaded, scheduling initial capture - content.js:428');
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('[Omega_KG] Tab hidden, capturing conversation - content.js:435');
...
window.addEventListener('beforeunload', () => {
  console.log('[Omega_KG] Page unloading, capturing conversation - content.js:442');
  capture.captureConversation();
});