...
def test_new_relationships_execute_without_error(mock_driver_class, mock_neo4j_driver):
    ...
    with patch("omega_kg.neo4j_schema.settings") as mock_settings:
        ...
        assert any("TRIGGERS" in q for q in calls), "TRIGGERS relationship should be created"
        assert any("BELONGS_TO" in q for q in calls), "BELONGS_TO relationship should be created"
        assert any("LinearIssue" in q for q in calls), "LinearIssue node should be created"
