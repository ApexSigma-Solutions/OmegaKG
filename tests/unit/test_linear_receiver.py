...
class TestSignatureVerification:
    ...
    @pytest.mark.asyncio
    async def test_verify_signature_alternate_header(self, mock_request, generate_signature):
        payload_bytes, signature = generate_signature
...
class TestReceiveLinearEvent:
    ...
    async def test_receive_event_persists_parsed_payload(
        ...
        with patch("omega_kg.routers.linear_receiver.RawLinearEvent", return_value=mock_event) as mock_constructor:
    ...
    @pytest.mark.asyncio
    async def test_receive_event_parses_external_timestamp(self, mock_request, test_payload):
        ...
        signature = hmac.new(settings.linear_webhook_secret.encode("utf-8"), payload_bytes, hashlib.sha256).hexdigest()
        ...
        with patch("omega_kg.routers.linear_receiver.RawLinearEvent", return_value=mock_event) as mock_constructor:
            await receive_linear_event(request=mock_request, db=mock_db, verification=(payload_bytes, signature))

        call_kwargs = mock_constructor.call_args[1]
        ext_ts = call_kwargs["external_timestamp"]
        assert isinstance(ext_ts, datetime)
        # Normalize to UTC for comparison
        assert ext_ts.astimezone(timezone.utc).isoformat().startswith("2020-01-01T00:00:00")

    @pytest.mark.asyncio
    async def test_receive_event_handles_invalid_timestamp_gracefully(self, mock_request, test_payload):
        payload = dict(test_payload)
        payload["data"] = dict(test_payload["data"])
    ...
    @pytest.mark.asyncio
    async def test_receive_event_stores_parsed_body_not_bytes(self, mock_request, generate_signature):
        payload_bytes, signature = generate_signature
        mock_request.headers = {"Linear-Signature": signature}
        mock_request.body.return_value = payload_bytes
    ...
    @pytest.mark.asyncio
    async def test_receive_event_persistence_failure_returns_500(self, mock_request, generate_signature):
        payload_bytes, signature = generate_signature
        mock_request.headers = {"Linear-Signature": signature}
        mock_request.body.return_value = payload_bytes
        ...
        with patch("omega_kg.routers.linear_receiver.RawLinearEvent", return_value=mock_event):
            with pytest.raises(HTTPException) as exc_info:
                await receive_linear_event(request=mock_request, db=mock_db, verification=(payload_bytes, signature))
