...
class TestStorePending:
    ...
    async def test_store_pending_with_invalid_node_label(self, vector_store):
        ...
        with pytest.raises(ValueError, match="not in SUPPORTED_NODE_TYPES"):
            await vector_store.store_pending(
                message_id=456,
                node_label="InvalidLabel"
            )

    @pytest.mark.asyncio
    async def test_store_pending_duplicate_handling(self, vector_store, mock_asyncpg_pool):
...
class TestMarkFailed:
    ...
    @pytest.mark.asyncio
    async def test_mark_failed_without_retry_increment(self, vector_store, mock_asyncpg_pool):
        """Test marking record as failed without incrementing retry"""
        mock_conn = mock_asyncpg_pool.acquire.return_value.__aenter__.return_value
        
        await vector_store.mark_failed(
            vector_id=123,
    ...
    @pytest.mark.asyncio
    async def test_mark_failed_with_retry_increment(self, vector_store, mock_asyncpg_pool):
...
class TestFetchPendingBatch:
    ...
    @pytest.mark.asyncio
    async def test_fetch_pending_batch_with_results(self, vector_store, mock_asyncpg_pool):
    ...
    @pytest.mark.asyncio
    async def test_fetch_pending_batch_custom_size(self, vector_store, mock_asyncpg_pool):
...
class TestGetVectorStatus:
    ...
    @pytest.mark.asyncio
    async def test_get_vector_status_all_valid_statuses(self, vector_store, mock_asyncpg_pool):
...
class TestGetVectorStore:
    ...
    async def test_get_vector_store_subsequent_calls(self):
        ...
        import omega_kg.vector_store
        mock_pool = AsyncMock()
        omega_kg.vector_store.vector_store = omega_kg.vector_store.VectorStore(mock_pool)
...
class TestConcurrency:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_store_pending_calls(self, vector_store, mock_asyncpg_pool):
    ...
    async def test_concurrent_fetch_batch_calls(self, vector_store, mock_asyncpg_pool):
        ...
        tasks = [
            vector_store.fetch_pending_batch(batch_size=10)
            for _ in range(3)
        ]
