...
class TestCheckNodes:
    ...
    def test_sample_query_executed_when_tasks_exist(self, mock_print, mock_driver):
        ...
        def run_side_effect(query, *args, **kwargs):
            """
            Selects and returns a prepared mock result that corresponds to the supplied Cypher query string.
            
            Parameters:
                query (str): The Cypher query used to choose which mock to return.
            
            Returns:
                Mock: `count_result` if `query` starts with "MATCH (t:Task) RETURN count"; `sample_result` if `query` starts with "MATCH (t:Task) RETURN t LIMIT 5"; `empty_result` if `query.strip()` starts with "MATCH (n) RETURN"; otherwise a new generic `Mock`.
            """
