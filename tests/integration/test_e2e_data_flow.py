...
async def test_e2e_complete_data_flow(
    ...
    assert len(search_results) > 0, \
        "Step 4: Search should return results"
    
    # Find our result (should be first with score 1.0)
    our_result = next(
        (r for r in search_results if r["message_id"] == message_id),
        None
    )
    assert our_result is not None, \
        "Step 4: Search should return our test conversation"
    assert our_result["score"] == 1.0, \
        "Step 4: Exact match should have score of 1.0"
...
async def test_e2e_error_handling_recovery(
    ...
    status = await vector_store.get_vector_status(vector_id)
    assert status == VectorStatus.FAILED, \
        "Vector should be marked as failed"
