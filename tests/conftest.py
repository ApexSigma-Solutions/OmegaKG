...
def pytest_addoption(parser):
    ...
    parser.addoption("--performance-threshold", action="store", default=1.0, type=float, help="Max duration (s) for perf tests")
    parser.addoption("--use-containers", action="store_true", default=True, help="Use Docker containers for integration tests")

def pytest_configure(config):
    """Register markers and load env vars."""
    config.addinivalue_line("markers", "slow: marks tests as slow")
    config.addinivalue_line("markers", "integration: marks tests requiring DB containers")
...
def mock_file_system(tmp_path):
    ...
    return {
        ...
        "write": lambda name, content: (test_dir / name).write_text(content, encoding="utf-8")
    }
