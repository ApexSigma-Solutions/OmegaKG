...
class RepositoryAnalyzer:
    ...
    def _scan_file_system(self):
        ...
        for file_path in self.root_path.rglob("*"):
            if file_path.is_file():
                ...
                try:
                    if ext in ['.py', '.md', '.json', '.yaml', '.yml', '.toml', '.ini', '.cfg']:
        ...
        print(f"   📈 Files: {total_files}, Lines: {total_lines}, Extensions: {len(file_types)}")
    ...
    def _scan_config_fragmentation(self):
        ...
        print(f"   📄 Found {len(config_files_found)} config files, {len(pytest_files)} pytest.ini files")
    ...
    def _scan_legacy_references(self):
        ...
        # Scan all text files
        text_extensions = {'.py', '.md', '.json', '.yaml', '.yml', '.toml', '.ini', '.cfg', '.txt'}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Repository Inventory & Dependency Scan")
    parser.add_argument("--root", default=".", help="Repository root path (default: current directory)")
    parser.add_argument("--output", default="reports/repository_inventory.json", help="Output file path")
    ...
    try:
        ...
        print(f"🔧 Config Fragmentation: {'Yes' if results['config_fragmentation']['fragmentation_detected'] else 'No'}")
