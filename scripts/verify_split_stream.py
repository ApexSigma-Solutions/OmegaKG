...
class SplitStreamVerifier:
    ...
    def _verify_neo4j_schema(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "exec", "apexsigma.neo4j.db", "cypher-shell", "-u", "neo4j", "-p", "password", 
                 "CALL db.schema()"],
                capture_output=True,
                text=True,
                timeout=10
    ...
    def _verify_cross_reference(self):
        ...
        postgres_ok = self.results["postgres_schema"].get("omega_vectors_1024_exists", False)
        neo4j_ok = self.results["neo4j_schema"].get("postgres_id_properties_found", False)
        ...
        if postgres_ok and neo4j_ok:
            ...
            print(f"   ⚠️  Cross-reference may be inconsistent")
    ...
    def _assess_verification_status(self):
        ...
        criteria = {
            "postgres_schema_valid": self.results["postgres_schema"].get("omega_vectors_1024_exists", False),
            "neo4j_schema_valid": self.results["neo4j_schema"].get("postgres_id_properties_found", False),
            "cross_reference_consistent": self.results["cross_reference"]["consistency_check"]["status"] == "CONSISTENT",
            "no_orphaned_issues": self.results["orphaned_records"]["detection_status"] == "BASIC_CHECK_PASSED"
        }
...
"""
        ...
        if all(self.results['verification_criteria'].values()):
            report += "- ✅ **All checks passed!** Data pipeline is properly configured.\n"
...
def main():
    ...
    parser.add_argument("--output", default="reports/data_verification_report.md", help="Output file path")
    ...
    try:
        ...
        print("="*60)
        print(f"🗄️  Postgres Schema: {'✅ Valid' if results['verification_criteria']['postgres_schema_valid'] else '❌ Invalid'}")
        print(f"🕸️  Neo4j Schema: {'✅ Valid' if results['verification_criteria']['neo4j_schema_valid'] else '❌ Invalid'}")
        print(f"🔗 Cross-Reference: {'✅ Consistent' if results['verification_criteria']['cross_reference_consistent'] else '❌ Inconsistent'}")
        print(f"👻 Orphaned Records: {'✅ Clean' if results['verification_criteria']['no_orphaned_issues'] else '❌ Issues'}")
