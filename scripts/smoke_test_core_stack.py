...
class InfrastructureSmokeTester:
    ...
    def _monitor_endpoint_health(self, end_time: float):
        ...
        for service_name, service_config in self.services.items():
            ...
            if not port_status:
                print(f"   ⚠️  Port {service_config['port']} not available for {service_name}")
                continue
            ...
            if check_count > 0:
                endpoint_stats[service_name]["success_rate"] = (success_count / check_count) * 100
                endpoint_stats[service_name]["avg_response_time"] = total_response_time / check_count
    ...
    def _check_system_resources(self):
        ...
        try:
            ...
            print(f"   📊 CPU: {cpu_percent}%, Memory: {memory.percent}%, Disk: {round((disk.used / disk.total) * 100, 2)}%")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if state.get("RestartCount", 0) > 0:
                        print(f"   ⚠️  {container_name} has restarted {state.get('RestartCount')} times")
                    else:
    ...
    def _assess_overall_status(self):
        ...
        if successful_criteria == total_criteria:
            ...
            print(f"   ⚠️  {successful_criteria}/{total_criteria} criteria met - Infrastructure is DEGRADED")
        else:
            self.results["overall_status"] = "UNHEALTHY"
            print(f"   ❌ {successful_criteria}/{total_criteria} criteria met - Infrastructure is UNHEALTHY")
...
"""
        ...
        for service_name, stats in self.results['endpoint_health'].items():
            if stats['checks']:
                ...
                report += f"- **Avg Response Time:** {stats['avg_response_time']:.3f}s\n"
...
def main():
    ...
    parser.add_argument("--output", default="reports/smoke_test_report.md", help="Output file path")
    ...
    try:
        ...
        print(f"📦 Containers: {'✅ Healthy' if results['success_criteria']['containers_running'] else '❌ Issues'}")
        print(f"🌐 Endpoints: {'✅ Healthy' if results['success_criteria']['endpoints_healthy'] else '❌ Issues'}")
        print(f"🔄 Crash Loops: {'✅ None' if results['success_criteria']['no_crash_loops'] else '❌ Detected'}")
