...
class TestDataSeeder:
        ...
        async with self.neo4j_driver.session() as session:
            for session_data in chat_sessions:
                ...
                if result:
                    ...
                    logger.error(f"Failed to create ChatSession: {session_data['session_id']}")
    ...
    async def _ensure_pgvector_extension(self) -> None:
        ...
        async with self.pg_conn.cursor() as cursor:
            ...
            await cursor.execute(create_table_query)
            logger.info(f"Ensured {VECTOR_TABLE_NAME} table exists with pgvector extension")
    ...
    async def create_relationships(self) -> int:
        ...
        async with self.pg_conn.cursor() as cursor:
            ...
            for rel in relationships:
                ...
                logger.info(f"Created relationship: {rel['from_session_id']} -> {rel['to_conversation_id']}")
    ...
    async def verify_data(self) -> Dict[str, Any]:
        ...
        async with self.pg_conn.cursor() as cursor:
            ...
            if result:
                ...
                logger.info(f"Relationships verification: Found {count}/3 relationships")
            else:
                verification_results["relationships"] = {
                    "expected": 3,
                    "found": 0,
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info(f"Neo4j ChatSessions: {verification_results['neo4j_sessions']['found']}/{verification_results['neo4j_sessions']['expected']}")
            logger.info(f"PostgreSQL Conversations: {verification_results['postgresql_conversations']['found']}/{verification_results['postgresql_conversations']['expected']} {verification_results['postgresql_conversations']['status']}")
            logger.info(f"pgvector Embeddings: {verification_results['pgvector_embeddings']['found']}/{verification_results['pgvector_embeddings']['expected']} {verification_results['pgvector_embeddings']['status']}")
            logger.info(f"Relationships: {verification_results['relationships']['found']}/{verification_results['relationships']['expected']} {verification_results['relationships']['status']}")
            logger.info("=" * 60)
