...
class HousekeepingManager:
    ...
    def clean_temp_dir(self, temp_dir: Path, description: str):
        ...
        if self.dry_run:
            ...
            self.log_action(f"Would clean: {temp_dir} ({files_count} files, {dirs_count} dirs) - {description}")
        else:
    ...
    def check_and_move_diagnostics(self):
        ...
        root_files = list(self.root_dir.glob("*.json")) + list(self.root_dir.glob("*.md"))
...
Co-Authored-By: Claude <noreply@anthropic.com>"""
            ...
            # Push if requested
            if push:
                subprocess.run(["git", "push", "origin", "beta"], cwd=self.root_dir, check=True)
