...
def verify_imports():
    ...
    try:
        from omega_kg.intelligence import Codex, Mirmir, Constraint, Verdict, IntelligenceError
    ...
    try:
        ...
        with codex._driver.session() as session:
            session.run("""
                ...
                MERGE (r)-[:GOVERNS]->(c)
            """, context=test_context, id=test_id)
        ...
        if not verdict.approved:
            logger.warning("⚠️  Verdict: VETOED (Note: Bootstrap logic might default to Approve or Warn)")
        else:
        ...
        with codex._driver.session() as session:
            ...
            session.run("MATCH (c:Context {name: $ctx}) DETACH DELETE c", ctx=test_context)
...
def main():
    ...
    parser.add_argument("--mode", choices=["static", "live"], default="live", help="Verification mode")
