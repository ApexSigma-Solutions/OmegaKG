...
class LinearSyncVerifier:
    ...
    async def test_linear_to_obsidian_webhook(self):
        ...
        logger.info(f"  Payload: {json.dumps(webhook_payload, indent=2, default=str)[:200]}...")
        ...
        async with AsyncSessionLocal() as session:
            ...
            logger.info("Step 2: Stored webhook in PostgreSQL (event_id=%d)", db_event.id)
        ...
        if linear_file.exists():
            ...
            logger.info(f"  Frontmatter: {json.dumps(post.metadata, indent=2, default=str)}")

            # Verify linear_id in frontmatter
            assert post.metadata.get("linear_id") == TEST_ISSUE_ID, "linear_id mismatch"
            assert post.metadata.get("linear_identifier") == TEST_ISSUE_IDENTIFIER, "linear_identifier mismatch"
...
"""
        ...
        with patch("omega_kg.linear_client.httpx.AsyncClient") as mock_client:
            ...
            mock_instance.post.return_value.json = MagicMock(return_value={
                "data": mock_linear_response
            })
            ...
            with patch("omega_kg.smart_parser.settings") as mock_settings:
                ...
                with patch("omega_kg.smart_parser.VaultUtils") as mock_vault:
                    ...
                    if result:
                        ...
                        logger.info(f"  GraphQL Operation: CreateIssue")
                        ...
                        updates = mock_vault.return_value.update_note_frontmatter.call_args[0][1]
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            results["obsidian_to_linear"] = await self.test_obsidian_to_linear_smart_parser()
        except Exception as e:
