...
async def check_postgres_connection():
    ...
    try:
        ...
        print(f"[OK] PostgreSQL connected successfully")
...
def check_obsidian_vault():
    ...
    if vault_path.exists():
        ...
        if os.access(vault_path, os.W_OK):
            print(f"  [OK] Vault is writable")
        else:
            print(f"  [FAIL] Vault is not writable")
            return False

        # Check for Linear subdirectory
        linear_dir = vault_path / "Linear"
        if linear_dir.exists():
            print(f"  [OK] Linear subdirectory exists")
        else:
            print(f"  [WARN] Linear subdirectory does not exist (will be created)")
...
async def check_database_schema():
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            if table_exists:
                print(f"[OK] Table 'raw_linear_events' exists")
                ...
                column_dict = {row[0]: {'column_name': row[0], 'data_type': row[1], 'is_nullable': row[2]} for row in columns}
                for col_name, expected_type in required_columns.items():
                    if col_name in column_dict:
                        ...
                        if expected_type in actual_type:
                            ...
                            print(f"    [WARN] {col_name}: {actual_type} (expected {expected_type})")
                    else:
                ...
                if 'ix_raw_linear_events_processed_received_at' in index_names:
                    print(f"  [OK] Composite index exists")
                else:
                    print(f"  [WARN] Composite index not found")

                return True
            else:
                print(f"[FAIL] Table 'raw_linear_events' does not exist")
                return False
    except Exception as e:
