...
async def create_malformed_event(session: AsyncSession, test_name: str) -> RawLinearEvent:
...
async def verify_error_handling(session: AsyncSession, event_id: int) -> bool:
    ...
    checks = {
        ...
        "Error logged": "error" in event.error_log.lower() if event.error_log else False,
    }
...
async def verify_system_continuity(session: AsyncSession) -> bool:
    ...
    if stats['processed'] >= 1:
        print(f"  [OK] System continues processing after errors")
        return True
    else:
        print(f"  [FAIL] System stopped processing after errors")
        return False
