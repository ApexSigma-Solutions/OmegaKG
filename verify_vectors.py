...
async def verify_vector():
    ...
    try:
        with driver.session() as session:
            ...
            if found_self:
                print("[SUCCESS] RAG verification passed: Task is retrievable via vector search.")
            else:
                print("[WARNING] Task was not found in top 5 similarity results (unexpected for self-search).")
