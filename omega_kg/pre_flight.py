...
def pre_flight_checks() -> bool:
    ...
    try:
        import omega_kg
        logger.info(f"[OK] omega_kg package version: {getattr(omega_kg, '__version__', 'unknown')}")
        ...
        logger.critical("="*60)
        logger.critical("")
        logger.critical("ERROR: The omega_kg package is not installed in this environment.")
        logger.critical("")
    ...
    if missing_deps:
        logger.error(f"[ERROR] Missing critical dependencies: {', '.join(missing_deps)}")
