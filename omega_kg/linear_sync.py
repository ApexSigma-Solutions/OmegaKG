...
class LinearSync:
    ...
    def __init__(self, vault_path: Optional[Path] = None, linear_client: Optional[LinearClient] = None):
        """
    ...
    def map_linear_state_to_vault_status(self, state_name: str, state_type: str) -> str:
        ...
        # Default fallback
        logger.warning(f"Unknown Linear state: {state_name} ({state_type}), defaulting to Backlog")
        return "Backlog"
    ...
    def update_local_note(self, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            issue_id, state_name, state_type, identifier = self.extract_issue_data(payload)
            ...
            if not file_path:
                ...
                logger.warning(f"No file found for linear_id={issue_id}, trying identifier={identifier}")
    ...
    def create_from_note(self, note_path: str) -> str:
        ...
        if not description:
            ...
            logger.warning(f"[create_from_note] Empty description, using fallback")

        # Status: Map vault status to Linear state if needed
        vault_status = str(metadata.get("status", "backlog"))
        state_id = self._map_vault_status_to_linear_state(vault_status, str(self.team_id))
        ...
        try:
            ...
            result = asyncio.run(self.linear_client.create_issue(
                title=str(title),
            ...
            identifier = result.get("identifier", issue_id) if isinstance(result, dict) else issue_id

            if not issue_id:
                logger.error(f"[create_from_note] API response missing issue ID: {result}")
                raise LinearAPIError(
                    f"Failed to create Linear issue: No issue ID in response"
                )
...
"""
        ...
        issue_id, state_name, state_type, identifier = sync.extract_issue_data(test_payload)
