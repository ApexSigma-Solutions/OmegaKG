...
from omega_kg.models.terminal import TerminalCommandData, TerminalCaptureResponse, TerminalEvent, Base
...
if settings.terminal_noise_patterns:
    ...
    for p in patterns:
        try:
            ...
            logger.error(f"Invalid regex pattern in TERMINAL_NOISE_PATTERNS: '{p}' - {e}")
...
async def capture_terminal_command(
    ...
    Processing (Neo4j/Embedding) happens asynchronously via worker.
    """
    ...
    new_event = TerminalEvent(
        ...
        processed=False # Worker will pick this up
    )
    ...
    try:
        ...
        logger.error(f"Failed to persist terminal event: {e}")
