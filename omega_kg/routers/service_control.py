...
async def control_service(
    ...
    # Map common names
    if "ingest" in service_key: service_key = "ingest"
    elif "memos" in service_key: service_key = "memos"
    ...
    if action == "stop":
        ...
        if _is_port_in_use(config["port"]):
             return {"status": "already_running", "service": config["name"], "message": "Port in use"}
        ...
        # Stop first
        await control_service(ServiceAction(service_name=service_key, action="stop"), _token)
        # Then start
        return await control_service(ServiceAction(service_name=service_key, action="start"), _token)
