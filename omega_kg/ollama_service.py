    ...
    try:
        ...
        for i in range(max_retries):
            if check_ollama_health():
                logger.info(f"✓ Ollama service started successfully (took {i+1} attempts)")
                return True
            time.sleep(2)
...
def get_ollama_status() -> dict:
    ...
    try:
        ...
        try:
            response = requests.get(
                f"{settings.ollama_host_url}/api/ps",
                timeout=2
            )
