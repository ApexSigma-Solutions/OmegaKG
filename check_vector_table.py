...
async def check_vector_status():
    print(f"Connecting to DB: {settings.postgres_server}:{settings.postgres_port}/{settings.postgres_db}")
    try:
        ...
        for r in rows:
            print(f"ID: {r['id']}, MsgID: {r['message_id']}, Label: {r['node_label']}, Status: {r['status']}")
